...
export const LoginCard: React.FC<LoginCardProps> = ({ onSuccess }) => {
  ...
  useEffect(() => {
    if (cardRef.current) {
      const cleanup = createTiltEffect(cardRef.current, 8);
      return cleanup;
    }
  }, []);

  useEffect(() => {
    if (user && user.isAuthenticated && onSuccess) {
      onSuccess();
    }
  }, [user, onSuccess]);
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await signInWithEmail(
        formData.email,
        formData.password,
        formData.isSignUp ? formData.name : '',
        formData.isSignUp
      );
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center pointer-events-auto">
      ...
      <div ref={cardRef} className="relative z-10 flex items-center justify-center">
        <GlassmorphismCard
          className="p-5 max-w-md w-full shadow-2xl animate-float"
          variant="primary"
          glowEffect
        >
            ...
            <div className="py-1 flex justify-center">
               <button
                ...
                className="relative w-full py-2 px-4 bg-gradient-to-r from-blue-600 to-purple-600 rounded-xl text-white font-semibold 
                         ...
                         transform hover:scale-105 hover:shadow-2xl hover:shadow-blue-500/50 flex items-center justify-center space-x-3"
              >
                <GoogleIcon />
            ...
            {showEmailForm && !otpSent && (
              <form onSubmit={handleEmailSubmit} className="space-y-2">
                ...
                <div className="flex items-center justify-center space-x-2 text-sm">
                  ...
                  <button
                    type="button"
                    onClick={() => setFormData({ email: '', password: '', name: '', isSignUp: !formData.isSignUp })}
